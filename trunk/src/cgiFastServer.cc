...
server::stackThread(void *data)
{
	...
	while (true)
	{
		...
		if (res == -1)
			throw baseEx(ERRMODULE_CGIFASTSERVER, FASTSERVEREX_STACKTHREAD, ERR_LIBDODO, FASTSERVEREX_ACCEPTFAILED, CGIFASTSERVEREX_ACCEPTFAILED_STR, __LINE__, __FILE__);
...
{
	if (!isFastCgi())
		throw baseEx(ERRMODULE_CGIFASTSERVER, FASTSERVEREX_LISTEN, ERR_LIBDODO, FASTSERVEREX_ISCGI, CGIFASTSERVEREX_ISCGI_STR, __LINE__, __FILE__);
...
#endif
	{
		...
		while (true)
		{
			if (FCGX_Accept_r(&request) == -1)
				throw baseEx(ERRMODULE_CGIFASTSERVER, FASTSERVEREX_LISTEN, ERR_LIBDODO, FASTSERVEREX_ACCEPTFAILED, CGIFASTSERVEREX_ACCEPTFAILED_STR, __LINE__, __FILE__);
