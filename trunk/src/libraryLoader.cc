...
#endif
	if (handle == NULL)
		throw baseEx(ERRMODULE_LIBRARYLOADER, LIBRARYLOADEREX_OPEN, ERR_DYNLOAD, 0, dlerror(), __LINE__, __FILE__);
}
...
#ifndef DL_FAST
	if (dlclose(handle) != 0)
		throw baseEx(ERRMODULE_LIBRARYLOADER, LIBRARYLOADEREX_CLOSE, ERR_DYNLOAD, 0, dlerror(), __LINE__, __FILE__);
#endif
}
