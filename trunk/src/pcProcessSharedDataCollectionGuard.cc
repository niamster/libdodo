/***************************************************************************
 *            pcProcessSharedDataCollectionGuard.cc
 *
...
dataCollectionGuard::del(unsigned long position)
{
	...
	if (getShare(position))
		...
		throw exception::basic(exception::ERRMODULE_PCPROCESSSHAREDDATACOLLECTIONGUARD, DATACOLLECTIONGUARDEX_DEL, exception::ERRNO_LIBDODO, DATACOLLECTIONGUARDEX_NOTFOUND, PCPROCESSSHAREDDATACOLLECTIONGUARDEX_NOTFOUND_STR, __LINE__, __FILE__);
}
						 ...
						 void          *data)
{
	...
	if (getShare(position))
		...
		throw exception::basic(exception::ERRMODULE_PCPROCESSSHAREDDATACOLLECTIONGUARD, DATACOLLECTIONGUARDEX_SET, exception::ERRNO_LIBDODO, DATACOLLECTIONGUARDEX_NOTFOUND, PCPROCESSSHAREDDATACOLLECTIONGUARDEX_NOTFOUND_STR, __LINE__, __FILE__);
}
...
{
	dodoList<pc::shared::__info>::iterator i(shares.begin()), j(shares.end());
	for (; i != j; ++i)
