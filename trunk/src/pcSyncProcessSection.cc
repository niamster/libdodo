...
section::section(int a_key) : semaphore(new __lock__),
							  autogenerated(false),
							  acquired(false)
{
#ifdef XSI_IPC
	...
	if (semaphore->keeper == -1) {
		...
		throw exception::basic(exception::MODULE_PCSYNCPROCESSSECTION, SECTIONEX_SECTION, exception::ERRNO_ERRNO, errno, strerror(errno), __LINE__, __FILE__);
	}
...
section::section() : semaphore(new __lock__),
					 autogenerated(false),
					 acquired(false)
{
}
...
section::close()
{
	...
	if (semaphore->keeper != -1) {
		if (autogenerated) {
			if (semctl(semaphore->keeper, 0, IPC_RMID) == -1)
				throw exception::basic(exception::MODULE_PCSYNCPROCESSSECTION, SECTIONEX_CLOSE, exception::ERRNO_ERRNO, errno, strerror(errno), __LINE__, __FILE__);
			else {
				if (acquired) {
					semaphore->operations[0].sem_op = 1;
	...
	if (semaphore->keeper != NULL) {
		...
		if (autogenerated)
			if (sem_unlink(semaphore->key.data()) == -1)
				throw exception::basic(exception::MODULE_PCSYNCPROCESSSECTION, SECTIONEX_CLOSE, exception::ERRNO_ERRNO, errno, strerror(errno), __LINE__, __FILE__);
	}
...
section::open(int a_key)
{
	...
	semaphore->keeper = semget(semaphore->key, 1, S_IRUSR | S_IWUSR | IPC_CREAT);
	if (semaphore->keeper == -1)
		throw exception::basic(exception::MODULE_PCSYNCPROCESSSECTION, SECTIONEX_OPEN, exception::ERRNO_ERRNO, errno, strerror(errno), __LINE__, __FILE__);
...
section::remove(int a_key)
{
	...
	if (keeper == -1)
		throw exception::basic(exception::MODULE_PCSYNCPROCESSSECTION, SECTIONEX_REMOVE, exception::ERRNO_ERRNO, errno, strerror(errno), __LINE__, __FILE__);
...
#ifdef XSI_IPC
	if (semaphore->keeper == -1)
		throw exception::basic(exception::MODULE_PCSYNCPROCESSSECTION, SECTIONEX_ACQUIRE, exception::ERRNO_LIBDODO, SECTIONEX_NOTOPENED, PCSYNCPROCESSSECTIONEX_NOTOPENED_STR, __LINE__, __FILE__);

	semaphore->operations[0].sem_op = -1;

	if (semop(semaphore->keeper, semaphore->operations, 1) != 0)
		throw exception::basic(exception::MODULE_PCSYNCPROCESSSECTION, SECTIONEX_ACQUIRE, exception::ERRNO_ERRNO, errno, strerror(errno), __LINE__, __FILE__);
...
#ifdef XSI_IPC
	if (semaphore->keeper == -1)
		throw exception::basic(exception::MODULE_PCSYNCPROCESSSECTION, SECTIONEX_RELEASE, exception::ERRNO_LIBDODO, SECTIONEX_NOTOPENED, PCSYNCPROCESSSECTIONEX_NOTOPENED_STR, __LINE__, __FILE__);

	semaphore->operations[0].sem_op = 1;

	if (semop(semaphore->keeper, semaphore->operations, 1) != 0)
		throw exception::basic(exception::MODULE_PCSYNCPROCESSSECTION, SECTIONEX_RELEASE, exception::ERRNO_ERRNO, errno, strerror(errno), __LINE__, __FILE__);
