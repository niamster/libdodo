	...
	if (force)
	{
		...
		if (::lstat(newPath.c_str(), &st) != -1)
		{
			if (!S_ISLNK(st.st_mode))
				throw baseEx(ERRMODULE_TOOLSFILESYSTEM, FILESYSTEMEX_SYMLINK, ERR_LIBDODO, FILESYSTEMEX_WRONGFILENAME, FILESYSTEMEX_WRONGFILENAME_STR, __LINE__, __FILE__, newPath);
			else
				   ...
				   bool force)
{
	if (::mkdir(path.c_str(), toRealPermission(permissions)) == -1)
	{
		if (force && (errno == EEXIST))
		{
			...
			if (!S_ISDIR(st.st_mode))
				throw baseEx(ERRMODULE_TOOLSFILESYSTEM, FILESYSTEMEX_MKDIR, ERR_LIBDODO, FILESYSTEMEX_NOTADIR, FILESYSTEMEX_NOTADIR_STR, __LINE__, __FILE__, path);
		}
		else
			throw baseEx(ERRMODULE_TOOLSFILESYSTEM, FILESYSTEMEX_MKDIR, ERR_ERRNO, errno, strerror(errno), __LINE__, __FILE__, path);
	}
							...
							int permissions)
{
	if (::mkdir(path.c_str(), toRealPermission(permissions)) == -1)
	{
		if (errno == EEXIST)
		{
			...
			if (!S_ISDIR(st.st_mode))
				throw baseEx(ERRMODULE_TOOLSFILESYSTEM, FILESYSTEMEX_MKDIRRECURSIVE, ERR_LIBDODO, FILESYSTEMEX_NOTADIR, FILESYSTEMEX_NOTADIR_STR, __LINE__, __FILE__, path);
		}
		else
		{
...
filesystem::followSymlink(const dodoString &path)
{
	...
	if (!S_ISLNK(st.st_mode))
		throw baseEx(ERRMODULE_TOOLSFILESYSTEM, FILESYSTEMEX_SYMLINK, ERR_LIBDODO, FILESYSTEMEX_WRONGFILENAME, FILESYSTEMEX_WRONGFILENAME_STR, __LINE__, __FILE__, path);
...
filesystem::getFileContents(const dodoString &path)
{
	...
	if (!S_ISREG(st.st_mode))
		throw baseEx(ERRMODULE_TOOLSFILESYSTEM, FILESYSTEMEX_GETFILECONTENTS, ERR_LIBDODO, FILESYSTEMEX_WRONGFILENAME, FILESYSTEMEX_WRONGFILENAME_STR, __LINE__, __FILE__, path);
...
filesystem::getFileContentsArr(const dodoString &path)
{
	...
	if (!S_ISREG(st.st_mode))
		throw baseEx(ERRMODULE_TOOLSFILESYSTEM, FILESYSTEMEX_GETFILECONTENTSARR, ERR_LIBDODO, FILESYSTEMEX_WRONGFILENAME, FILESYSTEMEX_WRONGFILENAME_STR, __LINE__, __FILE__, path);
...
filesystem::basename(const dodoString &path)
{
	if (path.size() >= MAXPATHLEN)
		throw baseEx(ERRMODULE_TOOLSFILESYSTEM, FILESYSTEMEX_LASTNAME, ERR_LIBDODO, FILESYSTEMEX_TOOLONGPATH, FILESYSTEMEX_TOOLONGPATH_STR, __LINE__, __FILE__, path);
...
{
	if (path.size() >= MAXPATHLEN)
		throw baseEx(ERRMODULE_TOOLSFILESYSTEM, FILESYSTEMEX_DIRNAME, ERR_LIBDODO, FILESYSTEMEX_TOOLONGPATH, FILESYSTEMEX_TOOLONGPATH_STR, __LINE__, __FILE__, path);
				  ...
				  bool force)
{
	...
	else
	{
		if (force)
		{
			...
			throw baseEx(ERRMODULE_TOOLSFILESYSTEM, FILESYSTEMEX_COPY, ERR_LIBDODO, FILESYSTEMEX_WRONGFILENAME, FILESYSTEMEX_WRONGFILENAME_STR, __LINE__, __FILE__, to);
	}
					 ...
					 bool force)
{
	...
	if (force)
		...
		throw baseEx(ERRMODULE_TOOLSFILESYSTEM, FILESYSTEMEX_COPYDIR, ERR_LIBDODO, FILESYSTEMEX_WRONGFILENAME, FILESYSTEMEX_WRONGFILENAME_STR, __LINE__, __FILE__, to);
